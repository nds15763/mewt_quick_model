...
// Audio features interface for emotion classification
...
export function classifyEmotion(features: AudioFeatures): EmotionClassification | null {
  const { zeroCrossingRate, spectralCentroid, spectralRolloff, energy, rms } = features;
  
  // Helper function to get emotion by id
  const getEmotion = (id: string): Emotion | undefined => emotions.find(e => e.id === id);
  const getCategory = (id: string): EmotionCategory | undefined => emotionCategories.find(c => c.id === id);
  ...
  let bestMatch: { emotionId: string; confidence: number } | null = null;

  // Classification rules for each emotion
  const rules: Array<{ emotionId: string; rule: () => number }> = [
...
export function classifyEmotionCategory(features: AudioFeatures): string | null {
...
export function getCategoryById(id: string): EmotionCategory | undefined {
